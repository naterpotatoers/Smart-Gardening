...
export const createPlant = (req, res) => {
  ...
  const queryString =
    "INSERT INTO plants(commonName, scientificName, growingSeason, weeksBeforeLastFrost, sunlight, tempMax, tempMin, humidityMax, humidityMin, phMax, phMin, id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  const query = mysql.format(queryString, [
    plant.commonName,
    plant.scientificName,
    plant.growingSeason,
    plant.weeksBeforeLastFrost,
    plant.sunlight,
    plant.tempMax,
    plant.tempMin,
    plant.humidityMax,
...
export const updatePlant = (req, res) => {
  ...
  if (weeksBeforeLastFrost) {
    const queryString =
      "UPDATE plants SET weeksBeforeLastFrost = ? WHERE id = ?";
